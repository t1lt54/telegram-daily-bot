...
На Render файловая система не подходит для надёжного постоянного хранения данных между деплоями, поэтому для продакшена лучше потом вынести подписчиков в внешнюю базу, например PostgreSQL. Пока для первого запуска и тестов локально это нормально.
